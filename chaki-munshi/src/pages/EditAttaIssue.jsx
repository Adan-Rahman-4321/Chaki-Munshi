...
export default function EditAttaIssue() {
  ...
  useEffect(() => {
    Promise.all([
      api.getCustomers(),
      api.getAttaIssue(id)
    ]).then(([custRes, issueRes]) => {
      if (custRes.success) {
        setCustomers(custRes.data);
      }
      if (issueRes.success) {
        const issue = issueRes.data;
        setCustomerId(issue.customerId.toString());
        setQuantity(issue.quantity.toString());
        setRatePerKg(issue.ratePerKg.toString());
        setPaymentMethod(issue.paymentMethod);
        setPaidAmount(issue.paidAmount?.toString() || '');
      }
    })
    .catch((err) => {
      console.error(err);
  ...
  const handleSubmit = (e) => {
    ...
    api.updateAttaIssue(id, payload)
      .then(async (res) => {
        if (res.success) {
          Swal.fire({
            icon: 'success',
            title: isRtl ? 'کامیابی!' : 'Success!',
            text: isRtl ? 'آٹا کی تقسیم کامیابی سے اپ ڈیٹ ہو گئی' : 'Flour issue updated successfully',
            confirmButtonText: isRtl ? 'ٹھیک ہے' : 'OK',
            confirmButtonColor: '#6d5b00'
          }).then(() => {
            navigate(`/invoice/flour/${id}`);
          });
        } else {
  ...
  return (
    <div className="space-y-stack-md animate-fade-in max-w-md mx-auto">
      <section className="flex items-center gap-2">
        ...
        <div>
          <h2 className="text-lg font-bold text-on-surface">Edit Flour Issue</h2>
          <p className="urdu-font text-[10px] text-primary leading-none mt-1">آٹا کی تقسیم میں تبدیلی</p>
        </div>
      </section>
      ...
      <div className="bg-surface-container-low border border-outline-variant rounded-xl p-stack-md shadow-sm select-none">
        ...
        <p className="text-on-surface text-xs mt-1">Modify the flour distribution details below.</p>
      </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="flex flex-col items-start leading-tight">
                <span className="text-sm font-bold">Update Issue</span>
                <span className="text-[9px] opacity-80 urdu-font -mt-1">تقسیم اپ ڈیٹ کریں</span>
              </div>
            </>
          )}
        </button>
